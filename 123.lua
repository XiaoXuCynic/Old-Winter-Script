...
local AimTransparencySlider = TabHandles.Real:Slider({
    Title = "圆圈透明度",
    Desc = "设置瞄准圈的透明度",
...
local SmoothnessSlider = TabHandles.Real:Slider({
    Title = "平滑度",
    Desc = "设置自瞄平滑度 (值越小越快)",
...
local Lighting = game:GetService("Lighting")
local RunServiceSky = game:GetService("RunService")
...
local function toggleStars(enabled)
    if enabled then
        ...
        starsConnection = RunServiceSky.RenderStepped:Connect(function()
...
local resetButton = TabHandles.Sky:Button({
    ...
    Callback = function()
        ...
        skyColor = originalValues.SkyColor or Color3.fromRGB(135, 206, 235)
        ambientColor = originalValues.Ambient or Color3.fromRGB(200, 200, 200)
        outdoorAmbient = originalValues.OutdoorAmbient or Color3.fromRGB(150, 150, 150)
        fogEnabled = false
